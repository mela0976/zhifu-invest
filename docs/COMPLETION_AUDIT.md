...
| 引薦人確認會員與認購 | 完成 | Docker admin E2E；Apps Script HtmlService 操作與 audit 寫入 |
...
| 潛在投資人名單與永久業績歸屬 | 完成 | 單筆與原子式 CSV 匯入、phone/email 去重、來源／隱私證據、首次有效導入者鎖定、會員連結衝突、`acquisitionAttributionSnapshot` 與 commission 快照分離、`lead-export-v1` 對帳 CSV |
| 每日投資摘要與電子報 | 程式完成 | 站內摘要固定可用；LINE／Email 各自明示同意、撤回取消未送 LINE、每次重試前重驗、台北日期冪等、MailApp quota 延後續送；未聲稱真實收件 |
| 投資內容與專案媒合 | 完成 | video／article／project_update 發布與公開安全閘門；媒合只用明示產業、票額、資格與逐案權限，排除未發布／撤回／關閉／未開放／過期專案並回傳理由 |
| LINE 通知政策 | 程式完成 | routine 自動、拒絕／退款／bulk 人工確認、首次失敗後再重試三次再轉待辦；未聲稱真實送達 |
| LINE Login／webhook gateway | 程式完成 | state／nonce、member resolution fail closed、raw body HMAC、D1 durable retry／去重／30 天清理；Worker route tests 含名單、內容、摘要與真實瀏覽器 DTO |
...
| Apps Script／Sheets operations | 程式完成 | signed envelope、缺 member ID fail closed、同 project lock＋flush、Google allowlist＋TOTP、引薦／分潤／Prospect／Content／Digest schema、MailApp quota、兩種單一 trigger 與 mobile CSV 後台 |
| 會員資料室與顧問預約 | 程式完成 | 企業／團隊／資金用途／財務、已核准 AI／專家報告 metadata、會員與後台預約列表 |
| 自動化回歸 | 通過 | Node 61、Worker 23、Apps Script 54、Playwright 12；Pages build、Worker typecheck／dry-run 與 diff check |
| 本機 Docker | 已運行 | `zhifu-invest:local` image `sha256:5043314…a1c25`；`zhifu-invest-app-1` healthy；`http://127.0.0.1:4173` 5-route smoke 與保留舊 named volume 的 Playwright 12/12 通過 |
...
| 引薦人與至少一位備援管理員 Google 帳號 | allowlist、Google 兩步驟驗證、兩組 TOTP、管理員實機登入與到期／鎖定測試 |
...
| 已同意的 Email 測試收件匣與 LINE 測試會員 | 驗證每日摘要實際送達、撤回後停止、重跑不重複、配額延後續送；API／queue success 不等於真人已收到 |
