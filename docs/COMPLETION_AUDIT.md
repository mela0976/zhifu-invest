...
| 範圍 | 結果 | 驗證方式 |
|---|---|---|
| Mobile-first 公開站、會員中心、雪芬姐儀表板 | 完成 | 390px 首頁、啟用、會員資料室／預約、後台預約與認購 Playwright journeys |
...
| LINE Login／webhook gateway | 程式完成 | state／nonce、member resolution fail closed、raw body HMAC、D1 durable retry／去重／30 天清理、13 個 Workers tests |
...
| Apps Script／Sheets operations | 程式完成 | signed envelope、缺 member ID fail closed、同 project lock＋flush、Google allowlist＋TOTP、preflight／單一 trigger、16 個 VM tests |
| 會員資料室與顧問預約 | 程式完成 | 企業／團隊／資金用途／財務、已核准 AI／專家報告 metadata、會員與後台預約列表 |
| 本機 Docker | 已運行 | `http://127.0.0.1:4173` health、5-route smoke、5 Playwright journeys |
